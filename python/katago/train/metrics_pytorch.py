...
class Metrics:
    ...
    def loss_ownership_samplewise(self, pred_pretanh, target, weight, mask, mask_sum_hw, global_weight):
        ...
        assert pred_pretanh.shape == (n, 1, self.pos_len_y, self.pos_len_x)
    ...
    def loss_futurepos_samplewise(self, pred_pretanh, target, weight, mask, mask_sum_hw, global_weight):
        ...
        n = pred_pretanh.shape[0]
        assert pred_pretanh.shape == (n, self.num_futurepos_values, self.pos_len_y, self.pos_len_x)
        assert target.shape == (n, self.num_futurepos_values, self.pos_len_y, self.pos_len_x)
        assert mask.shape == (n, self.pos_len_y, self.pos_len_x)
        assert mask_sum_hw.shape == (n,)
