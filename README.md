...
The quick summary is:
  ...
  * The CUDA+CUDNN backend can also work well for NVIDIA GPUs. It has faster startup than TensorRT and is competitive on transformers if using CUDNN >= 9.8.0.
...
More in detail:
  ...
  * TensorRT is similar to CUDA, but uses NVIDIA's TensorRT framework to run the neural network with more optimized kernels. For modern NVIDIA GPUs it should work whenever CUDA does, and will usually be the fastest backend, though it has much longer startup times on every launch. This fork requires TensorRT 11 or newer and has a fully ONNX-based backend using strongly typed graphs with explicit mixed precision; TensorRT 11.2.1 with CUDA 13.3 is the currently validated combination.
