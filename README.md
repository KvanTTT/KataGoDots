...
More in detail:
  ...
  * TensorRT is similar to CUDA, but uses NVIDIA's TensorRT framework to run the neural network with more optimized kernels. For modern NVIDIA GPUs it should work whenever CUDA does, and will usually be the fastest backend, though it has much longer startup times on every launch. TensorRT 10 and 11 are supported. TensorRT 11 builds strongly typed networks with an FP16 trunk and FP32 normalization and heads when FP16 is enabled. For transformer models, recent CUDA and TensorRT versions are recommended; older TensorRT versions can be outperformed by CUDA+CUDNN.
