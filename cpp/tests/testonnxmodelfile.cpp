...
map<string, string> makeMeta(int modelVersion, int numPolicyChannels) {
  ...
  meta["katago.numInputChannels"] = Global::intToString(NNModelVersion::getNumSpatialFeatures(modelVersion, false));
  meta["katago.numInputGlobalChannels"] = Global::intToString(NNModelVersion::getNumGlobalFeatures(modelVersion, false));
...
void Tests::runOnnxModelFileTests(const string& scratchDir, const string& modelFile) {
  ...
  // ---- Happy path, model version 15 ----
  {
    ...
    testAssert(desc.numInputChannels == NNModelVersion::getNumSpatialFeatures(15, false));
    testAssert(desc.numInputGlobalChannels == NNModelVersion::getNumGlobalFeatures(15, false));
    testAssert(!desc.isDotsGame);
  ...
  // ---- A Dots model says so, since its inputs are featurized differently ----
  {
    map<string, string> meta = makeMeta(15, 2);
    meta["katago.dotsGame"] = "true";
    onnx::ModelProto model = makeModel(meta);
    ModelDesc desc;
    writeAndLoad(model, path, desc);
    testAssert(desc.isDotsGame);
  ...
  expectLoadError(
    withMeta("katago.build.nnXLen", Global::intToString(NNPos::MAX_BOARD_LEN_X + 1)),
  ...
  expectLoadError(withMeta("katago.dotsGame", "maybe"), path, "is not a boolean", "bad dotsGame boolean");
  expectLoadError(
  ...
  if(modelFile != "") {
    ...
    testAssert(desc.isDotsGame == srcDesc.isDotsGame);
