...
{
  {
    ...
    auto [hist, board] = sgf->setupBoardAndHistAssumeLegal(initialRules, nextPla, 40,BoardHistoryModes(false,false));
...

  {
    ...
    Player nextPla;
    Rules initialRules = sgf->getRulesOrFailAllowUnspecified(Rules::getTrompTaylorish());
    auto [hist, board] = sgf->setupBoardAndHistAssumeLegal(initialRules, nextPla, 43,BoardHistoryModes(false,false));
