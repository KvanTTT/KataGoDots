...
HASH: 5C26A060FA78FD93FFF559C72BD7C6A4
 ...
 1 X X X O O . O O O
...

  {
    ...
    auto [hist, board] = sgf->setupBoardAndHistAssumeLegal(initialRules, nextPla, turnIdxToSetup,BoardHistoryModes(false,false));
    string expected = R"%%(
...

  {
    ...
    for(int i = 0; i<numBoards; i++) {
      for(int j = 0; j<rules.size(); j++) {
        ...
        BoardHistory hist(board,nextPla,currentRules,0,BoardHistoryModes(false,false));
        BoardHistory hist2(board2,nextPla,currentRules,0,BoardHistoryModes(false,false));
