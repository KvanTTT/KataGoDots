...
void BookHash::getHashAndSymmetry(const BoardHistory& hist, int repBound, BookHash& hashRet, int& symmetryToAlignRet, vector<int>& symmetriesRet, int bookVersion) {
  ...
  for(int symmetry = 0; symmetry < numSymmetries; symmetry++) {
    ...
    histsBySym.emplace_back(boardsBySym[symmetry], hist.initialPla, hist.rules, hist.initialEncorePhase, hist.alwaysComputePassAliveUnderSuicideRules);
...
BookParams BookParams::loadFromCfg(ConfigParser& cfg, int64_t maxVisits, int64_t maxVisitsForLeaves) {
  ...
  cfgParams.bonusForWLPV1 = cfg.getOrDefaultDouble("bonusForWLPV1", 0.0, 1000000.0, 0.0);
  cfgParams.bonusForWLPV2 = cfg.getOrDefaultDouble("bonusForWLPV2", 0.0, 1000000.0, 0.0);
  cfgParams.bonusForWLPVFinalProp = cfg.getOrDefaultDouble("bonusForWLPVFinalProp", 0.0, 1.0, 0.5);
  cfgParams.bonusForBiggestWLCost = cfg.getOrDefaultDouble("bonusForBiggestWLCost", 0.0, 1000000.0, 0.0);
  cfgParams.bonusBehindInVisitsScale = cfg.getOrDefaultDouble("bonusBehindInVisitsScale", 0.0, 1000000.0, 0.0);
  cfgParams.scoreLossCap = cfg.getDouble("scoreLossCap",0.0,1000000.0);
  cfgParams.earlyBookCostReductionFactor = cfg.getOrDefaultDouble("earlyBookCostReductionFactor", 0.0, 1.0, 0.0);
  cfgParams.earlyBookCostReductionLambda = cfg.getOrDefaultDouble("earlyBookCostReductionLambda", 0.0, 1.0, 0.5);
  ...
  cfgParams.adjustedVisitsWLScale = cfg.getOrDefaultDouble("adjustedVisitsWLScale", 0.0, 1000000.0, 0.05);
  cfgParams.maxVisitsForReExpansion = cfg.getOrDefaultDouble("maxVisitsForReExpansion", 0.0, 1e50, 0.0);
  cfgParams.visitsScale = cfg.getOrDefaultDouble("visitsScale", std::numeric_limits<double>::min(), std::numeric_limits<double>::max(), (double)(maxVisits + 1) / 2);
  cfgParams.visitsScaleLeaves = cfg.getOrDefaultDouble("visitsScaleLeaves", std::numeric_limits<double>::min(), std::numeric_limits<double>::max(), (double)maxVisitsForLeaves);
