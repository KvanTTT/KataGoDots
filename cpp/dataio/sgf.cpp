...
void Sgf::loadAllUniquePositions(
  ...
  std::function f = [&samples](const PositionSample& sample, const BoardHistory& hist, const string& comments) {
    (void)hist;
    (void)comments;
    ...
    for(size_t j = 0; j<buf.size(); j++) {
      ...
      if(!tolerateIllegalMoves && (!tolerantLegal || simpleKoBanned)) {
        throw StringError(
          ...
          Location::toString(moveLoc, board) + " SGF trace (branches 0-indexed): " + buildTrace(i)
        );
      }
      ...
      if(tolerateIllegalMoves && !tolerantLegal) {
        cerr << "WARNING: Skipping illegal move in " << fileName << " effective turn "
             ...
             << Location::toString(moveLoc, board)
             << " SGF trace (branches 0-indexed): " << buildTrace(i) << endl;
...
BoardHistory CompactSgf::setupInitialBoardAndHist(const Rules& initialRules, Player& nextPla, const BoardHistoryModes& modes) const {
  ...
  auto hist = BoardHistory(board,nextPla,initialRules,0,modes);
...
std::pair<BoardHistory, Board> CompactSgf::setupBoardAndHistAssumeLegal(const Rules& initialRules, Player& nextPla, int64_t turnIdx, const BoardHistoryModes& modes)
  const {
  BoardHistory hist = setupInitialBoardAndHist(initialRules, nextPla, modes);
...
std::pair<BoardHistory, Board> CompactSgf::setupBoardAndHistTolerant(
  ...
  int64_t turnIdx,
  bool preventEncore,
  const BoardHistoryModes& modes) const {
  BoardHistory hist = setupInitialBoardAndHist(initialRules, nextPla, modes);
  Board boardWithMoves(hist.initialBoard);
