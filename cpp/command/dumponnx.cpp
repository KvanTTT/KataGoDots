...
int MainCmds::dumponnx(const vector<string>& args) {
  ...
  if(nnXLen < 2 || nnXLen > NNPos::MAX_BOARD_LEN_X || nnYLen < 2 || nnYLen > NNPos::MAX_BOARD_LEN_Y) {
    cerr << "Board size must be between 2x2 and " << NNPos::MAX_BOARD_LEN_X << "x" << NNPos::MAX_BOARD_LEN_Y << endl;
    return 1;
  }
