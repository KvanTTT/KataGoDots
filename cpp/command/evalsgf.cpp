...
int MainCmds::evalsgf(const vector<string>& args) {
  ...
  auto setUpBoardUsingRules = [&board,&nextPla,&hist,overrideKomi,&sgf,&extraMoves,&alwaysComputePassAliveUnderSuicideRules](const Rules& initialRules, int moveNum) {
    ...
    hist = sgf->setupInitialBoardAndHist(initialRules, nextPla, alwaysComputePassAliveUnderSuicideRules);
  ...
  for(int moveNum = moveNumStart; moveNum <= moveNumEnd; moveNum++) {
    ...
    if(dumpNpzInputTo != "") {
      ...
      NNInputs::fillRowVN(7, board, hist, nextPla, nnInputParams, nnXLen, nnYLen, inputsUseNHWC, binaryInputNCHW.data, globalInputNC.data, false);
