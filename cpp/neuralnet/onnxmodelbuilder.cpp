...
const char* const META_DOTS_GAME = "katago.dotsGame";
...
Result build(
  ...
  //Only written for a Dots model, so that a Go model stays loadable by a KataGo that doesn't know the key,
  //while a Dots one is refused by it instead of being fed Go inputs
  if(desc.isDotsGame)
    addMeta(META_DOTS_GAME, Global::boolToString(desc.isDotsGame));
...
LoadResult load(
  ...
  // The game fixes the input encoding together with the model version, so it has to be known before
  // the channel counts are checked.
  descBuf.isDotsGame = reader.getBoolOr(META_DOTS_GAME, false);
  ...
  checkChannels("numInputChannels", descBuf.numInputChannels, NNModelVersion::getNumSpatialFeatures(descBuf.modelVersion, descBuf.isDotsGame));
  checkChannels("numInputGlobalChannels", descBuf.numInputGlobalChannels, NNModelVersion::getNumGlobalFeatures(descBuf.modelVersion, descBuf.isDotsGame));
  ...
  if(result.buildParams.nnXLen < 2 || result.buildParams.nnXLen > NNPos::MAX_BOARD_LEN_X ||
     result.buildParams.nnYLen < 2 || result.buildParams.nnYLen > NNPos::MAX_BOARD_LEN_Y)
    reader.fail(Global::strprintf(
      "graph board size %dx%d is outside the supported range 2x2 to %dx%d",
      result.buildParams.nnXLen, result.buildParams.nnYLen, NNPos::MAX_BOARD_LEN_X, NNPos::MAX_BOARD_LEN_Y));
