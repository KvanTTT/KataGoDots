...
BoardHistory::BoardHistory(const Board& board) : BoardHistory(board, P_BLACK, board.rules, 0, BoardHistoryModes(false,false)) {}
...
std::vector<Loc> BoardHistory::getReasonableMoves(
  ...
  if (board.is_finished) {
    // Check for finishing just in case and return no reasonable moves
    return result;
  }
