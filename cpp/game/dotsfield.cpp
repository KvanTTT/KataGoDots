...
int Location::getNextLocCWIndex(const Loc loc0, const Loc loc1, const int x_size) {
  const int diff = loc1 - loc0;
  const int stride = x_size + 1;

  if (diff == -1) return LEFT_TOP_INDEX;
  if (diff == -1 - stride) return TOP_INDEX;
  if (diff == -stride) return RIGHT_TOP_INDEX;
  if (diff == +1 - stride) return RIGHT_INDEX;
  if (diff == +1) return RIGHT_BOTTOM_INDEX;
  if (diff == +1 + stride) return BOTTOM_INDEX;
  if (diff == +stride) return LEFT_BOTTOM_INDEX;
  if (diff == -1 + stride) return LEFT_INDEX;
...
int Board::calculateOwnershipAndWhiteScore(Color* result, const Color groundColor) const {
  ...
  for (int y = 0; y < y_size; y++) {
    for (int x = 0; x < x_size; x++) {
      ...
      if (activeColor != C_EMPTY) {
        if (isGrounded(state) || groundColor == C_EMPTY) {
          ...
          if (groundColor == C_WHITE && placedDotColor == P_WHITE) {
            ownershipColor = P_BLACK;
            whiteCaptures++;
          } else if (groundColor == C_BLACK && placedDotColor == P_BLACK) {
...
void Board::undoDots(const MoveRecord& moveRecord) {
  ...
  for (auto it = moveRecord.bases.rbegin(); it != moveRecord.bases.rend(); ++it) {
    for (const auto& locStateCaptureDiff : it->rollback_locs_states_captures) {
      const Loc rollbackLoc = locStateCaptureDiff.getLoc();
      const State prevState = locStateCaptureDiff.getState();
      ...
      if (it->type != Base::Type::EMPTY) {
        ...
        if (isGrounded(currentState) && !isGrounded(prevState)) {
          if (getPlacedDotColor(currentState) != C_EMPTY) {
            if (getActiveColor(currentState) == P_BLACK) {
              whiteScoreIfBlackGrounds++;
            } else {
              blackScoreIfWhiteGrounds++;
            }
          }
          clearGrounded(rollbackLoc);
        }

        // Simulate unmaking the new captures diff and applying the old one
        ...
        if (rollbackCapturesDiff != UNINITIALIZED_CAPTURES_DIFF_DATA) {
          pos_hash ^= capturesDiffHash[rollbackCapturesDiff];
        }

        // Erase the placed dot
        if (const Color placedColor = getPlacedDotColor(prevState); placedColor != C_EMPTY && !isTerritory(prevState)) {
          pos_hash ^= ZOBRIST_BOARD_HASH[rollbackLoc][placedColor];
        }

        captures_diff_data[rollbackLoc] = rollbackCapturesDiff;
      }
    }

    numBlackCaptures -= it->black_captures_diff;
    numWhiteCaptures -= it->white_captures_diff;
  }
...
vector<Loc> Board::fillGrounding(const Loc loc) {
  ...
  [[maybe_unused]] const bool result = setGroundedAndUpdateGroundingScore(loc, pla);
  assert(result);
  ...
  while (!walkStack.empty()) {
    ...
    forEachAdjacent(currentLoc, [&](const Loc adjLoc) {
      if (setGroundedAndUpdateGroundingScore(adjLoc, pla)) {
        groundedLocs.push_back(adjLoc);
...
void Board::finishBaseGrounding(const vector<Base>& bases) {
  for(const auto& base: bases) {
    ...
    const Player pla = base.pla;
    for(const auto& rollback_locs_states_capture: base.rollback_locs_states_captures) {
      setGroundedAndUpdateGroundingScore(rollback_locs_states_capture.getLoc(), pla);
    }
  }
}

bool Board::setGroundedAndUpdateGroundingScore(const Loc loc, const Player pla) {
  const State state = getState(loc);
  if (isGrounded(state) || !isActive(state, pla)) {
    return false;
  }
  ...
  for (int y = 0; y < y_size; y++) {
    for (int x = 0; x < x_size; x++) {
      ...
      if (const State state = getState(loc); !isGrounded(state) && isActive(state, pla)) {
        int numCapturedDots, numFreedDots;
        getTerritoryLocations(opp, loc, true, numCapturedDots, numFreedDots);
        // Since we are starting from an active dot, at least one captured dot is expected even if dotsCaptureEmptyBases is true
        assert(numCapturedDots > 0);
...
Color Board::getColorsOfPotentialCapturing(const Loc loc, const int minNumberOfConnections) const {
  ...
  if (numBlackUnconnectedDots >= minNumberOfConnections) {
    result |= C_BLACK;
  }
  if (numWhiteUnconnectedDots >= minNumberOfConnections) {
    result |= C_WHITE;
  }
  return result;
}
...
void Board::getTerritoryLocations(const Player pla,
  ...
  assert(!grounding || (activeColor == opp && !isGrounded(state)));

  if (activeColor == opp) {
    if (const Color placedDotColor = getPlacedDotColor(state); placedDotColor == opp) {
      numCapturedDots++;
    } else if (placedDotColor == pla) {
      numFreedDots++;
    }
  }
  ...
  while (!walkStack.empty()) {
    ...
    forEachAdjacent(loc, [&](const Loc adj) {
      ...
      if (activeColor == opp) {
        assert(!grounding || !isGrounded(state));
        isAdjLegal = true;
        if (const Color placedDotColor = getPlacedDotColor(state); placedDotColor == opp) {
          numCapturedDots++;
        } else if (placedDotColor == pla) {
...
void Board::updateStatesAndAppendBase(
  ...
  const int capturesDiff = numCapturedDots + numFreedDots;
  assert(capturesDiff >= 0);

  int blackCapturesDiff = 0;
  int whiteCapturesDiff = 0;

  if (basePla == P_BLACK) {
    blackCapturesDiff = -numFreedDots;
    whiteCapturesDiff = numCapturedDots;
  } else if (basePla == P_WHITE) {
    blackCapturesDiff = numCapturedDots;
    whiteCapturesDiff = -numFreedDots;
  } else {
    assert(false && "Invalid base player");
  }
  ...
  for (const Loc& territoryLoc : territoryLocationsBuffer) {
    ...
    if (baseType != Base::Type::EMPTY) {
      ...
      if (prevCapturesDiff != UNINITIALIZED_CAPTURES_DIFF_DATA)
        pos_hash ^= capturesDiffHash[prevCapturesDiff];

      pos_hash ^= capturesDiffHash[capturesDiff];
    ...
    locStateAndCapturesDiffs.emplace_back(territoryLoc, prevState, prevCapturesDiff);
...
Board::LocStateAndCapturesDiff::LocStateAndCapturesDiff(const Loc newLoc, const State newState, const uint16_t newCapturesDiff) {
  packed = (static_cast<uint32_t>(newLoc) & LOC_BITS_MASK) |
           (static_cast<uint32_t>(newState) & STATE_BITS_MASK) << LOC_BITS_COUNT |
           (static_cast<uint32_t>(newCapturesDiff) & CAPTURES_BITS_MASK) << (LOC_BITS_COUNT + STATE_BITS_COUNT);
}

Loc Board::LocStateAndCapturesDiff::getLoc() const {
  return static_cast<Loc>(packed & LOC_BITS_MASK);
}

State Board::LocStateAndCapturesDiff::getState() const {
  return static_cast<State>((packed >> LOC_BITS_COUNT) & STATE_BITS_MASK);
}

uint16_t Board::LocStateAndCapturesDiff::getCapturesDiff() const {
