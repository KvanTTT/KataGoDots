...
vector<NNEvaluator*> Setup::initializeNNEvaluators(
  ...
  for(size_t i = 0; i<nnModelFiles.size(); i++) {
    ...
    bool inputsUseNHWC = backendPrefix != "opencl" && backendPrefix != "trt" && backendPrefix != "metal" && backendPrefix != "onnx" ;
...
vector<SearchParams> Setup::loadParams(
  ...
  for(int i = 0; i<numBots; i++) {
    ...
    params.excludeTerritoryAdjacentToAtari = getValueForBot<enabled_t>(cfg, "excludeTerritoryAdjacentToAtari", idxStr, enabled_t::False, enabled_t::Auto, enabled_t::Auto);
