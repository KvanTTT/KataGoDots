...
## Linux
   ...
   * Requirements
      ...
      * If using the TensorRT backend, install TensorRT 11.0 or newer and its matching CUDA Toolkit. The backend is validated with TensorRT 11.2.1 and CUDA 13.3. It is ONNX-only and uses TensorRT 11 strongly typed networks; the removed weak-typing builder path is not supported. A full SDK install is required for the C++ headers, libraries, and ONNX parser; the Python `pip` package alone is not sufficient.
...
## Windows
   ...
   * Requirements
      ...
      * If using the TensorRT backend, install the full TensorRT 11.0-or-newer Windows ZIP SDK and its matching CUDA Toolkit. The backend is validated with TensorRT 11.2.1 and CUDA 13.3. It builds only strongly typed networks through the internal ONNX emitter and has no legacy hand-built fallback. The TensorRT `pip` package contains the Python runtime but not the C++ development files needed by this build. Add the SDK's `bin` directory to `PATH` when running KataGo and pass its root as `-DTENSORRT_ROOT_DIR=<path>` when configuring CMake.
      * The TensorRT ONNX build path additionally requires protobuf. With vcpkg, the complete non-distributed dependency set is `vcpkg install zlib:x64-windows libzip:x64-windows protobuf:x64-windows`.
