...
Explanation of fields (including some optional fields not present in the above query):
   ...
   * `initialPlayer (player string)`: Optional. Specifies the player to use for analyzing the first turn (turn 0) of the game, which can be useful if `moves` is an empty list.
      * For Dots, a turn 0 that neither this nor `playerToMove` states is played by whichever player has fewer dots in `initialStones`, and by the first player when both have as many. The dots of a start position are a position rather than a sequence, so the order they are listed in makes no difference.
   * `playerToMove (player string)`: Optional. For Dots only. The player every analyzed position is evaluated for, `analyzeTurns` included: in Go the player to move follows from the moves, while in Dots either player may move at any point, so it's the client that knows whose move is being considered. Without it, a position is evaluated for the player whose turn it is by the alternation of the moves.
   * `dots (boolean)`: Optional. Whether the query is a game of Dots rather than of Go, which decides the default rules of the query and the way its moves and its players are named. Defaults to the `dots` of `CONFIG_FILE`; the model has to be a Dots one either way, so this states the game of a query rather than switching the engine between the two.
   ...
   * `timeControl (object)`: Optional. The clock the search of this query is played on, stated the way the `time_settings` and `time_left` commands of GTP state it, see [GTP_Extensions.md](GTP_Extensions.md). Without it, and with both times zeroed, the search is limited by `maxVisits` and by the config alone. All the fields are floats of seconds, from 0 to 1e25.
      * `mainTime` - the time of the whole game. On its own it is an absolute time control: the search spends the game time it has left as it sees fit.
      * `perMoveTime` - the time of every move, which is spent before the main time and is not banked when it's left, that is a Bronstein delay. This is how Dots is normally played.
      * `mainTimeLeft` - what the player to move has left of `mainTime`. Defaults to the whole of it, that is to a game that is just starting.
      * `perMoveTimeLeft` - what the player to move has left of `perMoveTime` of the move that is being searched. `0` means the controller doesn't track it, in which case the whole of it is assumed, and more than it is no extra time either, since only up to it is ever refunded. Defaults to the whole of it.
      * A stated clock is the whole of the limit of the search: the `maxVisits`, `maxPlayouts` and `maxTime` of `CONFIG_FILE`, which are the defaults for a query that states no limit at all, no longer apply, since they are far smaller than any clock and would stop the search almost at once and leave the time of the move unspent. A query that states `maxVisits`, or one of those three in `overrideSettings`, is limited by what it states as well, and the search stops at whichever limit comes first.
...
Current fields are:
   ...
   * `chosenMove`: Only on the final response of a position, that is when `isDuringSearch` is false. The move the engine itself would play in this position, chosen among the analyzed ones the way a move of a game is rather than by the evaluations alone (`chosenMoveTemperature` of `CONFIG_FILE` varies it), so that a client that plays the engine needs no second engine to ask for a move. A pass is reported as `"pass"`.
   * `resignReasonable`: For Dots only, and only on the final response of a position. Whether resigning is reasonable for the player the position is analyzed for. Resigning is a decision of a player rather than of the search, and the one of Dots follows from the position alone, so it is reported rather than left for a client to guess from the win rate.
