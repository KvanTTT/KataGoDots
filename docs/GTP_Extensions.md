...
In addition to a basic set of [GTP commands](https://www.lysator.liu.se/~gunnar/gtp/), KataGo supports a few additional commands:
   ...
   * `time_left [COLOR] TIME PERMOVETIMELEFT`
      * Under any time control, Go included, `COLOR` may be left out, as in `time_left 600 30`. The clock reported is then the one of the player to move, the same assumption `genmove` makes. The two forms are told apart by whether the first argument parses as a player at all, which a time never does.
   ...
   * `kata-time_settings KIND ...`
      ...
      * Supports all of the same settings as `kgs-time_settings` except includes additional time settings:
        ...
        * `default` takes no arguments and puts the engine back on the time control it started on, that is the one its config leaves it with. This is the only way back: the search limits a config states are not a time control and cannot be stated as one, so a controller that has to undo a time control of its own - a game played with a clock being followed by one played without - cannot use `none`, which would leave the engine with no limit at all rather than with the limits of its config. Only `kata-time_settings` takes it; `kgs-time_settings` does not.
   ...
   * `kata-list_time_settings`
      * Reports all time settings supported by `kata-time_settings`, separated by whitespace. Currently the list is `none absolute byoyomi canadian fischer fischer-capped bronstein default`.
