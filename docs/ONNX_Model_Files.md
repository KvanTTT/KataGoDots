...
| Key | Default | Meaning |
|---|---|---|
| `katago.dotsGame` | `false` | `true` for a Dots model, whose input channel counts differ from those of a Go model of the same version. A build without Dots support refuses a model that carries it, so `dumponnx` writes it only for Dots models. |
